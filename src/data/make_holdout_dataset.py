...
file_path = 'data/raw/holdout_final.pkl.zip'

def run(path):
    zip_file = ZipFile(path)
    df = pd.read_pickle(zip_file.open('holdout_final.pkl'))
    ...
    compression_opts = dict(method='zip',archive_name='clean_holdout.csv')
    df_clean.to_csv('data/out/clean_holdout.zip', index=False,compression=compression_opts)  
